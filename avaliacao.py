...
from pylatex import Document, NoEscape, Section, Subsection, Figure, Head, MiniPage, PageStyle, Foot,  LargeText, \
    MediumText, LineBreak, StandAloneGraphic, Package
...
def plot_graph(avals_t,int_i,int_s):
    '''Updates graph for LATEX inclusion'''
    q_i = []
    acum = 0
    answers_n = 0
    for i in range(int_i,int_s):
        info = list(map(int,list(filter(None, avals_t[i]))))
        acum += sum(info)
        answers_n += len(info)
    ...
    ax.set_title('Conceito e Número de Avaliações. Média {:04.2f}.'.format(acum/answers_n))
...
def generate_pdf(n_alunos, n_respostas, avals, cod, prof):
    '''Creates the body of the PDF file'''
